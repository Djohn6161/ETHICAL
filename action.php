<?php  
 //action.php  
    ...
    if(isset($_POST["product_id"])) {  
        ...
        $message = '';  
        if($_POST["action"] == "add") {  
           if(isset($_SESSION["newage"])) {  
                $is_available = 0;  
                foreach($_SESSION["newage"] as $keys => $values) {  
                    if($_SESSION["newage"][$keys]['product_id'] == $_POST["product_id"]) {  
                        $is_available++;  
                        $_SESSION["newage"][$keys]['product_quantity'] = $_SESSION["newage"][$keys]['product_quantity'] + $_POST["product_quantity"];  
                    }  
                }  
                if($is_available < 1)  {  
                    $item_array = array(  
        ...
        if($_POST["action"] == "quantity_change") {  
           ...
           {  
                if($_SESSION["newage"][$keys]['product_id'] == $_POST["product_id"])  
                {  
                     $_SESSION["newage"][$keys]['product_quantity'] = $_POST["quantity"];  
                }  
           }  
      }   
        $order_table .= '  
      ...
      $output = array(  
           ...
           'cart_item'   => count($_SESSION["newage"])  
      );  
